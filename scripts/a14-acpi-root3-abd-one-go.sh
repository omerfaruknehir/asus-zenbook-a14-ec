...
# ACPI ROOT3: own QCOM0427/ABD ROP1 at the ACPICA GenericSerialBus boundary
# and trace the firmware's virtual provider requests without fabricating data.
#
# Safety invariants:
#   - Image-only incremental build; NEVER make modules/modules_install
#   - no initramfs, no DTB
#   - prepare always clears/leaves GRUB next_entry UNARMED
#   - arm only sets one-shot GRUB state; it NEVER reboots
#   - diagnostic PID1 NEVER reboots automatically
set -euo pipefail

ACTION="${1:-prepare}"
KREL="7.1.5-a14-acpi-root3"
LOCALVER="-a14-acpi-root3"
ENTRY="ASUS Zenbook A14 — ACPI ROOT3 ABD GSBUS ($KREL)"
...
TRANSFORM="$ROOT/scripts/apply-a14-acpi-root3-abd.py"
...
arm(){
    need grub-reboot
    [[ -s "$KERNEL" ]] || die "ROOT3 kernel missing: $KERNEL"
    [[ -x "$INIT_HELPER" ]] || die "ROOT3 PID1 helper missing: $INIT_HELPER"
    ...
    [[ "$next" == "$ENTRY" ]] || die "failed to arm ROOT3: ${next:-missing}"
...
prepare(){
    ...
    [[ -f "$SRC/Makefile" && -f "$CONFIG" && -s "$IMAGE" && -s "$VMLINUX" ]] || die "compact ROOT2 build tree missing under $WORK"
    [[ -r "$TRANSFORM" ]] || die "missing ROOT3 ABD transform: $TRANSFORM"
    ...
    # ROOT2 is the required source baseline.
    grep -Fq 'A14_QCOM_WOA_ACPI_GPIO_EVENT_XLATE' "$acpigpio" || die "ROOT2 GPIO-event translation missing"
    grep -Fq 'A14_QCOM_SCM_ACPI_QCOM04DD' "$scm" || die "ROOT2 SCM ACPI support missing"
    grep -Fq 'QCOMEDK2", 0x8380' "$smmuq" || die "ROOT2 SMMU 0x8380 match missing"
    grep -Fq 'A14_GIO0_SAFE_REGISTRATION_V1' "$msm" || die "ROOT1 GIO0 registration fix missing"
    table="$(awk '/x1e80100_pinctrl_acpi_match\[\]/{f=1} f{print} f && /^};/{exit}' "$x1")"
    grep -Fq 'QCOM0C0C' <<<"$table" || die "GIO0 QCOM0C0C match missing"
    ! grep -Fq 'QCOM0C0D' <<<"$table" || die "IPC0 QCOM0C0D still incorrectly matches TLMM"

    say "A14_ACPI_ROOT3_STAGE=0 ABD-transform"
    ...
    grep -Fq 'A14_QCOM_ABD_GSBUS_TRACE_V1' "$abd" || die "ABD driver marker missing"
    ...
    grep -Fq 'status = AE_SUPPORT' "$abd" || die "trace-only explicit-failure policy missing"
    ...
    sudo -u "$OWNER" "$C" --disable DEBUG_INFO --file "$CONFIG"
    sudo -u "$OWNER" "$C" --enable DEBUG_INFO_NONE --file "$CONFIG"
    sudo -u "$OWNER" "$C" --disable DEBUG_INFO_BTF --file "$CONFIG"
    ...
    grep -aFq 'A14 ABD: QCOM0427 GenericSerialBus handler installed (trace-only)' "$VMLINUX" || die "compiled kernel lacks ABD handler marker"
    grep -aFq 'A14 ABD: request=' "$VMLINUX" || die "compiled kernel lacks ABD request trace"
...
append_file(){
    ...
    sync -f "$ROOTLOG" 2>/dev/null || sync
    [[ -z "$ESPLOG" ]] || sync -f "$ESPLOG" 2>/dev/null || sync
}
append_cmd(){
    local tmp=/run/a14-root3-capture.tmp
    "$@" >"$tmp" 2>&1 || true
    append_file "$tmp"
    rm -f "$tmp"
}
...
[[ -n "$ESPLOG" ]] && emit 'A14_ACPI_ROOT3_EFI_MIRROR=PASS' || emit 'A14_ACPI_ROOT3_EFI_MIRROR=FAIL'
emit "kernel=$(uname -a)"
emit "cmdline=$(cat /proc/cmdline 2>/dev/null || true)"
emit "root_mount=$(grep ' / ' /proc/mounts 2>/dev/null || true)"

# Let built-in ACPI/platform/I2C probes and dependent AML settle.
sleep 5
DM=/run/a14-root3-dmesg.txt
dmesg >"$DM" 2>&1 || true

if grep -q 'A14 ABD: QCOM0427 GenericSerialBus handler installed' "$DM"; then emit 'A14_ACPI_ROOT3_GSBUS_HANDLER=PASS'; else emit 'A14_ACPI_ROOT3_GSBUS_HANDLER=INCOMPLETE'; fi
if grep -q 'A14 ABD: request=' "$DM"; then emit 'A14_ACPI_ROOT3_ABD_REQUESTS=CAPTURED'; else emit 'A14_ACPI_ROOT3_ABD_REQUESTS=NONE_SEEN'; fi
if grep -qE 'No handler for Region \[ROP1\]|Region GenericSerialBus .* has no handler' "$DM"; then emit 'A14_ACPI_ROOT3_ROP1_NO_HANDLER=STILL_PRESENT'; else emit 'A14_ACPI_ROOT3_ROP1_NO_HANDLER=CLEARED'; fi
if [[ -d /sys/bus/acpi/devices/QCOM0427:00 ]]; then emit 'A14_ACPI_ROOT3_ABD_NAMESPACE=PASS'; else emit 'A14_ACPI_ROOT3_ABD_NAMESPACE=INCOMPLETE'; fi
if grep -q 'QTEC0001:00 0B05:0220 Keyboard' /proc/bus/input/devices 2>/dev/null; then emit 'A14_ACPI_ROOT3_KEYBOARD_PATH=PASS'; else emit 'A14_ACPI_ROOT3_KEYBOARD_PATH=INCOMPLETE'; fi
if grep -q 'qcom_scm: convention: smc arm 64' "$DM"; then emit 'A14_ACPI_ROOT3_SCM=PASS'; else emit 'A14_ACPI_ROOT3_SCM=INCOMPLETE'; fi
smmu_count=$(grep -c 'arm-smmu.*probing hardware configuration' "$DM" 2>/dev/null || true)
if [[ ${smmu_count:-0} -ge 2 ]]; then emit 'A14_ACPI_ROOT3_SMMU=PASS'; else emit "A14_ACPI_ROOT3_SMMU=INCOMPLETE count=${smmu_count:-0}"; fi
emit 'A14_ACPI_ROOT3_BACKENDS=UNIMPLEMENTED_BY_DESIGN'

emit '===== ABD / QCOM0427 NAMESPACE ====='
append_cmd sh -c 'for d in /sys/bus/acpi/devices/QCOM0427:* /sys/bus/platform/devices/QCOM0427:*; do [ -e "$d" ] || continue; echo "-- $d"; cat "$d/path" 2>/dev/null; ls -l "$d/physical_node" "$d/driver" 2>&1; done'
emit '===== PMGK / GLNK / UCSI NAMESPACE ====='
append_cmd sh -c 'for p in QCOM0C8E QCOM0C84 USBC000 PNP0CA0; do for d in /sys/bus/acpi/devices/$p*; do [ -e "$d" ] || continue; echo "-- $d"; cat "$d/path" 2>/dev/null; ls -l "$d/physical_node" "$d/driver" 2>&1; done; done'
emit '===== INPUT ====='
append_file /proc/bus/input/devices
emit '===== INTERRUPTS ====='
append_file /proc/interrupts
emit '===== IOMMU / SMMU ====='
append_cmd sh -c 'ls -l /sys/bus/platform/drivers/arm-smmu /sys/kernel/iommu_groups 2>&1; find /sys/kernel/iommu_groups -maxdepth 2 -type l -print 2>/dev/null | sort'
emit '===== HIGH SIGNAL DMESG ====='
append_cmd sh -c "dmesg | grep -Ei 'A14 ABD|QCOM0427|ROP1|GenericSerialBus|PRTC|PMGK|QCOM0C8E|UCSI|QCOM0C84|qcom_scm|arm-smmu|QCOM WoA GPIO|QTEC0001|error|fail'"
emit '===== FULL DMESG ====='
append_file "$DM"
...
printf '\nROOT3 capture complete. Automatic reboot is disabled.\nReport: %s\n' "$ROOTLOG" >/dev/tty0 2>/dev/null || true
printf 'ABD is trace-only in ROOT3; virtual provider backends are intentionally not fabricated.\n' >/dev/tty0 2>/dev/null || true
printf 'Use the keyboard/shell, then reboot manually when YOU choose.\n\n' >/dev/tty0 2>/dev/null || true
exec bash -i </dev/tty0 >/dev/tty0 2>&1
ROOT3_RUNTIME

    chmod 0755 "$INIT_HELPER"
    bash -n "$INIT_HELPER" || die "generated ROOT3 PID1 failed bash -n"
    rm -f "$ROOTLOG"
...
#!/bin/sh
exec tail -n +3 \$0
# ROOT3: factory ACPI, ABD/ROP1 GSBUS trace. NO DTB. NO initramfs.
...
EOF
    ...
    # Safety invariant: successful prepare remains UNARMED.
    grub-editenv /boot/grub/grubenv unset next_entry 2>/dev/null || true
    ...
    say "abd_mode=trace-only-explicit-AE_SUPPORT"
    say "physical_i2c_assumption=false"
    ...
    say "reboot_performed=false"
    say "To arm: sudo bash scripts/a14-acpi-root3-abd-one-go.sh arm"
