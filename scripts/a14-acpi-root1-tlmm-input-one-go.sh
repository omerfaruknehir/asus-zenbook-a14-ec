...
# Minimal no-initrd ACPI v2.1 bring-up: GIO0/TLMM + WoA GpioInt translation
# + I2C-HID keyboard, building Image only and capturing without local input.
set -euo pipefail
...
OUT="$WORK/root0-build"              # reuse the compact Image-only object tree
...
for c in git python3 make sha256sum awk grep sed findmnt blkid lsblk grub-probe grub-mkrelpath update-grub grub-reboot grub-editenv grub-script-check sync du df; do need "$c"; done
[[ "$(uname -r)" != 7.1.5-a14-acpi-full0 && "$(uname -r)" != 7.1.5-a14-acpi-root0 && "$(uname -r)" != "$KREL" ]] || die "run from the normal DT/rescue kernel"
[[ -f "$SRC/Makefile" && -f "$CONFIG" && -s "$IMAGE" ]] || die "compact ROOT0 build tree missing under $WORK; ROOT0 must be built first"
...
# Collect every EFI System Partition on the same NVMe. The PID1 helper will try
# them in order; failure of ESP logging never blocks the root log.
...
say "A14_ACPI_ROOT1_STAGE=0 evidence-derived-transform"
...
# Reuse the exact historical transform programs that produced the prior GIO0
# evidence, but do not switch branches or disturb the user's local worktree.
...
# Apply in the historically validated order. V1/V2 are diagnostic evidence;
# XLATE + SAFE are the functional changes. All transforms are fail-closed.
...
# Verify the exact semantics before compiling.
...
grep -Fq 'A14_GIO0_SAFE_REGISTRATION_V1' "$msm" || die "safe registration fix missing"
grep -Fq 'pctrl->chip.get_direction = NULL' "$msm" || die "eager direction scan suppression missing"
grep -Fq 'pctrl->chip.get_direction = a14_saved_get_direction' "$msm" || die "runtime direction callback restore missing"
grep -Fq 'A14_QCOM_WOA_ACPI_GPIO_XLATE' "$acpigpio" || die "WoA GpioInt translator missing"
grep -Fq 'qcom_woa_pdc_dsm_guid' "$acpigpio" || die "PDC DSM translator source missing"

# Keep only the drivers needed to prove TLMM -> GpioInt -> I2C-HID input in the
# already-small ROOT0 Image. No make modules and no initramfs.
...
say "A14_ACPI_ROOT1_STAGE=1 olddefconfig"
...
[[ -s "$IMAGE" ]] || die "ROOT1 Image missing"
image_sha="$(sha256sum "$IMAGE" | awk '{print $1}')"

# Prove the functional patch strings made it into built objects/vmlinux.
grep -aFq 'A14GIO0FIX: suppressing registration-time eager direction scan' "$OUT/vmlinux" || die "compiled kernel lacks GIO0 safe-registration fix"
grep -aFq 'QCOM WoA GPIO: virtual' "$OUT/vmlinux" || die "compiled kernel lacks WoA virtual GPIO translator"
...
# Capture directly from the mounted root. ROOTLOG is authoritative; ESP is a
# synchronous best-effort mirror. /run/a14-esp is created AFTER tmpfs /run is
# mounted, fixing ROOT0's previous hidden-mountpoint bug.
...
# Give deferred built-in probes a brief chance to settle; no userspace daemon is
# required for the ACPI/platform/I2C-HID driver-core path.
sleep 3
...
if grep -q 'x1e80100-tlmm' "\$ROOTLOG" 2>/dev/null && grep -qE 'QTEC0001|0B05|HID' /proc/bus/input/devices 2>/dev/null; then
...
# Clean previous experimental snippets. Keep normal distro DT entries untouched.
grub-editenv /boot/grub/grubenv unset next_entry 2>/dev/null || true
...
say "A14_ACPI_ROOT1_TLMM_INPUT=READY"
