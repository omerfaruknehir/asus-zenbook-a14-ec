#!/usr/bin/env bash
# Cumulative suspend A/B stage 2: exclude the already-cleared custom hardware
# prep hook plus the Aegis Hello system-sleep hook. Stop Aegis before the test
# so its camera work settles before suspend, then restore it after resume.

set -u

report=${A14_AEGIS_HOOK_AB_REPORT:-"$HOME/Downloads/a14-suspend-aegis-hook-ab-test.txt"}
...
for item in "$hardware_hook:$hardware_sha" "$aegis_hook:$aegis_sha"; do
    hook=${item%%:*}
    expected=${item##*:}
    sudo test -f "$hook" || fail "target hook is missing: $hook"
    actual=$(sudo sha256sum -- "$hook" | awk '{print $1}')
    [ "$actual" = "$expected" ] || fail "hook hash changed: $hook expected=$expected actual=$actual"
done

exec > >(tee "$report") 2>&1

printf '%s\n' 'A14 suspend A/B stage 2: hardware-prep and Aegis hooks excluded'
printf '%s\n' '=================================================================='
printf 'timestamp=%s\n' "$(date --iso-8601=ns)"
printf 'kernel=%s\n' "$(uname -r)"
...
printf '%s\n' 'state_changes_performed=temporary-hook-bind-masks-aegis-stop-and-suspend'
...
printf '\n===== MASKED TARGETS =====\n'
sudo stat -Lc 'path=%n type=%F mode=%a owner=%U:%G' -- "$hardware_hook" "$aegis_hook"
...
if [ "$completed" = true ]; then
    printf '%s\n' 'result=suspend-cycle-completed-with-hardware-and-aegis-hooks-excluded'
