...
# ACPI ROOT6: normal systemd userspace boot using the already-built ROOT5 kernel.
#
# Safety invariants:
#   - no kernel rebuild
...
ENTRY="ASUS Zenbook A14 — ACPI ROOT6 normal userspace via ROOT5 ($KREL)"
...
SNIPPET="/etc/grub.d/48_a14_acpi_root6_userspace"
ROOT5_REPORT="$OWNER_HOME/Downloads/a14-acpi-root5.txt"
ROOT6_NOTE="$OWNER_HOME/Downloads/a14-acpi-root6-userspace-note.txt"

for c in awk grep sed findmnt blkid lsblk grub-probe grub-mkrelpath update-grub grub-editenv grub-script-check sha256sum sync; do need "$c"; done

unarm(){
    grub-editenv /boot/grub/grubenv unset next_entry 2>/dev/null || true
    say "A14_ACPI_ROOT6_UNARMED=1"
...
arm(){
    ...
    [[ -r "$SNIPPET" ]] || die "ROOT6 GRUB snippet missing: $SNIPPET; run prepare first"
    grub-script-check /boot/grub/grub.cfg >/dev/null
    grub-reboot "$ENTRY"
    next="$(grub-editenv /boot/grub/grubenv list | sed -n 's/^next_entry=//p' | head -n1)"
    [[ "$next" == "$ENTRY" ]] || die "failed to arm ROOT6: ${next:-missing}"
    say "A14_ACPI_ROOT6_ARMED=1"
    say "next_entry=$next"
    say "reboot_performed=false"
...
menuentry '$ENTRY' {
    ...
    linux '$kernel_path' root=PARTUUID=$root_partuuid rw rootfstype=ext4 rootwait acpi=force earlycon=efifb,ram console=tty0 loglevel=8 ignore_loglevel printk.time=1 panic=0 clk_ignore_unused pd_ignore_unused cma=128M efi=noruntime stubble.dtb_override=false systemd.unit=multi-user.target
}
EOF
    chmod 0755 "$SNIPPET"
    ...
    {
        say "A14_ACPI_ROOT6_NOTE=prepared"
        say "kernel=$KERNEL"
        say "kernel_sha256=$(sha256sum "$KERNEL" | awk '{print $1}')"
        say "entry=$ENTRY"
        say "mode=normal_systemd_multi_user"
    ...
    chown "$OWNER:$OWNER" "$ROOT6_NOTE" 2>/dev/null || true
    sync

    say "A14_ACPI_ROOT6_PREPARE=PASS"
    say "kernel=$KERNEL"
    say "kernel_sha256=$(sha256sum "$KERNEL" | awk '{print $1}')"
    say "mode=normal_systemd_multi_user"
    ...
    say "reboot_performed=false"
    say "To arm: sudo bash scripts/a14-acpi-root6-userspace-one-go.sh arm"
}
