...
The ``on``/``off`` actions below therefore send the proven full 64-byte feature
report to product 0220.  Product 4543 remains available only for historical
comparison; it is not the Windows Fn-switch target.
"""
...
def main() -> int:
    ...
    # The proven Fn-switch target is 0220.  Keep 'all' explicit rather than
    # making an omitted product accidentally write to the auxiliary 4543 HID.
    ...
    if action not in actions or product_arg not in products:
        print(
            f"usage: {sys.argv[0]} {{status|functions|init|on|off}} [0220|4543|all]",
            file=sys.stderr,
        )
        return 2
    ...
    for node, product, name, driver in devices:
        ...
        if action in {"on", "off"} and product != FNLOCK_PRODUCT:
