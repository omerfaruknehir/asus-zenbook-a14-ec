...
# ACPI ROOT4: read-only SOSI ABI probe + provider-4 payload trace on ROOT3.
#
# Safety invariants:
#   - Image-only incremental build; no modules/modules_install
#   - no initramfs, no DTB
#   - SOSI is read only; ABD provider 4 still returns AE_SUPPORT
...
ENTRY="ASUS Zenbook A14 — ACPI ROOT4 SOSI+ABD-P4 trace ($KREL)"
...
TRANSFORM="$ROOT/scripts/apply-a14-acpi-root4-sosi.py"
...
prepare(){
    ...
    [[ -r "$TRANSFORM" ]] || die "missing ROOT4 transformer: $TRANSFORM"
    ...
    [[ -r "$abd" ]] || die "ROOT3 qcom_abd.c missing"
    grep -Fq 'A14_QCOM_ABD_GSBUS_TRACE_V1' "$abd" || die "ROOT3 ABD trace baseline missing"
    ...
    grep -Fq 'QCOMEDK2", 0x8380' "$SRC/drivers/iommu/arm/arm-smmu/arm-smmu-qcom.c" || die "ROOT2 SMMU match missing"

    say "A14_ACPI_ROOT4_STAGE=0 apply-SOSI-and-provider4-traces"
    ...
    grep -Fq 'A14_QCOM_ABD_PROVIDER4_TRACE_V2' "$abd" || die "provider-4 payload trace missing"
    grep -Fq 'A14 ABD P4 TX:' "$abd" || die "provider-4 hex dump missing"
    grep -Fq 'A14_QCOM_SOSI_READONLY_PROBE_V1' "$sosi" || die "SOSI probe missing"
    grep -Fq 'acpi_evaluate_integer(NULL, "\\_SB.SOSI"' "$sosi" || die "SOSI address is not firmware-derived"
    grep -Fq 'writes=0' "$sosi" || die "SOSI read-only marker missing"
    grep -Fq 'status = AE_SUPPORT' "$abd" || die "ROOT4 must keep provider backend explicit-fail"
    ...
    rm -f \
      "$OUT/drivers/acpi/arm64/qcom_abd.o" "$OUT/drivers/acpi/arm64/.qcom_abd.o.cmd" \
      "$OUT/drivers/acpi/arm64/qcom_sosi_probe.o" "$OUT/drivers/acpi/arm64/.qcom_sosi_probe.o.cmd"
    ...
    grep -aFq 'A14 ABD P4 TX:' "$VMLINUX" || die "compiled kernel lacks provider-4 payload trace"
    grep -aFq 'A14 SOSI: ACPI \_SB.SOSI=' "$VMLINUX" || die "compiled kernel lacks SOSI probe"
    grep -aFq 'A14 SOSI: full map copied' "$VMLINUX" || die "compiled kernel lacks SOSI full-map stage"
    ...
    say "A14_ACPI_ROOT4_STAGE=3 install-diagnostic-PID1-and-GRUB"
    {
        printf '%s\n' '#!/usr/bin/env bash'
        printf '%s\n' 'set -u'
        printf 'ROOTLOG=%q\n' "$ROOTLOG"
    ...
    cat >>"$INIT_HELPER" <<'ROOT4_RUNTIME'
PATH=/usr/sbin:/usr/bin:/sbin:/bin
ESPMNT=/run/a14-esp
printf '%s\n' 'A14_ACPI_ROOT4_INIT_ENTERED=1' >"$ROOTLOG"
...
for d in $ESP_CANDIDATES; do
    if mount -t vfat -o rw,sync "$d" "$ESPMNT" 2>/dev/null; then
        ...
        : >"$ESPLOG"
        printf 'A14_ACPI_ROOT4_EFI_MOUNT=PASS device=%s\n' "$d" >>"$ESPLOG"
...
append_cmd(){ local t=/run/a14-root4-capture.tmp; "$@" >"$t" 2>&1 || true; append_file "$t"; rm -f "$t"; }

emit 'A14_ACPI_ROOT4_DIRECT_ROOT_MOUNT=PASS'
if [[ -n "$ESPLOG" ]]; then emit 'A14_ACPI_ROOT4_EFI_MIRROR=PASS'; else emit 'A14_ACPI_ROOT4_EFI_MIRROR=FAIL'; fi
emit "kernel=$(uname -a)"
emit "cmdline=$(cat /proc/cmdline 2>/dev/null || true)"

sleep 6
dmesg >/run/a14-root4-dmesg.txt 2>&1 || true

emit '===== ROOT4 SUMMARY ====='
grep -q 'A14 SOSI: ACPI \\_SB.SOSI=' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_SOSI_EVAL=PASS' || emit 'A14_ACPI_ROOT4_SOSI_EVAL=INCOMPLETE'
grep -q 'A14 SOSI: header mapped' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_SOSI_HEADER=PASS' || emit 'A14_ACPI_ROOT4_SOSI_HEADER=INCOMPLETE'
if grep -q 'A14 SOSI: derived size accepted' /run/a14-root4-dmesg.txt; then
    emit 'A14_ACPI_ROOT4_SOSI_SIZE=PASS'
elif grep -q 'A14 SOSI: derived size rejected' /run/a14-root4-dmesg.txt; then
    emit 'A14_ACPI_ROOT4_SOSI_SIZE=REJECTED_SAFELY'
else
    emit 'A14_ACPI_ROOT4_SOSI_SIZE=INCOMPLETE'
fi
grep -q 'A14 ABD P4 TX:' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_PROVIDER4_TX=PASS' || emit 'A14_ACPI_ROOT4_PROVIDER4_TX=NO_REQUEST_SEEN'
grep -q 'A14 ABD: QCOM0427 GenericSerialBus handler installed' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_ABD_HANDLER=PASS' || emit 'A14_ACPI_ROOT4_ABD_HANDLER=INCOMPLETE'
grep -q 'qcom_scm: convention: smc arm 64' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_SCM0=PASS' || emit 'A14_ACPI_ROOT4_SCM0=INCOMPLETE'
grep -q 'arm-smmu.1.auto:.*preserved 0 boot mappings' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_SMMU=PASS' || emit 'A14_ACPI_ROOT4_SMMU=INCOMPLETE'
grep -q 'I2C HID v1.00 Keyboard \[QTEC0001:00 0B05:0220\]' /run/a14-root4-dmesg.txt && emit 'A14_ACPI_ROOT4_KEYBOARD=PASS' || emit 'A14_ACPI_ROOT4_KEYBOARD=INCOMPLETE'
emit 'A14_ACPI_ROOT4_PROVIDER4_BACKEND=TRACE_ONLY_AE_SUPPORT'
emit 'A14_ACPI_ROOT4_SOSI_WRITES=0'

emit '===== SOSI / ABD HIGH SIGNAL ====='
grep -E 'A14 SOSI:|A14 ABD:|A14 ABD P4|QCOM0427|ROP1|GenericSerialBus|PMGK|OEMN' /run/a14-root4-dmesg.txt >>"$ROOTLOG" 2>/dev/null || true
if [[ -n "$ESPLOG" ]]; then grep -E 'A14 SOSI:|A14 ABD:|A14 ABD P4|QCOM0427|ROP1|GenericSerialBus|PMGK|OEMN' /run/a14-root4-dmesg.txt >>"$ESPLOG" 2>/dev/null || true; fi
sync
...
emit '===== FULL DMESG ====='
append_file /run/a14-root4-dmesg.txt
emit 'A14_ACPI_ROOT4_CAPTURE_COMPLETE=1'
sync

printf '\nROOT4 capture complete. Automatic reboot is disabled.\n' >/dev/tty0 2>/dev/null || true
...
ROOT4_RUNTIME
    ...
    bash -n "$INIT_HELPER" || die "generated ROOT4 PID1 helper has shell syntax errors"
...
EOF
    ...
    say "abd_provider4_payload_trace=true"
    say "sosi_address_source=ACPI_\\_SB.SOSI"
    say "sosi_access=read-only"
    say "sosi_derived_size_cap=0x4000"
    say "provider4_backend=trace-only-AE_SUPPORT"
    ...
    say "reboot_performed=false"
    say "To arm: sudo bash scripts/a14-acpi-root4-sosi-one-go.sh arm"
}
