...
def restore_generated_hid_base() -> None:
    ...
    The composition helpers intentionally mutate hid_asus_ec.c in the working
    tree. A fast-forward pull does not replace that modified file, so a source
    generated by an older Fn-lock transform can contain the current Fn+F marker
    while missing the newer Fn-lock markers. Rebuild only that known generated
    state from the committed HEAD base, then apply current transforms in order.

    This recovery is deliberately gated on the generated profile marker and a
    Git checkout. It never resets an arbitrary unmarked hand-edited HID file.
    """
